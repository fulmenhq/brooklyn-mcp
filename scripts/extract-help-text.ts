...
const HELP_BLOCK_REGEX = /```help-text-([a-z-]+)(?:\r?\n)([\s\S]*?)```/g;
...
async function generateEmptyHelpFiles(): Promise<void> {
  // Ensure output directory exists
  await mkdir(OUTPUT_DIR, { recursive: true });

  // Generate empty TypeScript module for importing
  const tsContent = `// Auto-generated help text from markdown docs
// Do not edit this file directly - edit the markdown sources instead
// No help blocks found - generating empty module for build compatibility

// Empty help text lookup map
export const HELP_TEXT = {} as const;
`;
...
async function main() {
  try {
    ...
    console.log("📝 Extracting help blocks...");
    ...
    console.log(`📦 Extracted ${allBlocks.length} total help blocks`);

    if (allBlocks.length === 0) {
      console.log("ℹ️  No help blocks found - generating empty help files for build compatibility");
      await generateEmptyHelpFiles();
      console.log("✅ Empty help files generated for build compatibility");
      return;
    }

    console.log("🏗️  Generating help files...");
    await generateHelpFiles(allBlocks);

    console.log("✅ Help text extraction complete!");
    ...
    console.error("❌ Help text extraction failed:", error);
    process.exit(1);
  }
}
