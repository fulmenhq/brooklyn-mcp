...
async function embedVersion(): Promise<void> {
  console.log("🔧 Embedding version in source files...");

  try {
    execSync("bun run version:embed", { stdio: "inherit" });
    console.log("✅ Version embedding completed");
  } catch (error) {
    console.error("❌ Version embedding failed");
    throw error;
  }
}
...
async function commitVersionArtifacts(version: string): Promise<void> {
  ...
  if (status.isClean) {
    console.log("✅ No version artifacts to commit - working tree is clean");
    return;
  }

  if (!status.hasVersionArtifacts) {
    console.log("⚠️  Working tree has changes but no version artifacts detected");
    console.log("Files with changes:", status.files.join(", "));
    console.log("Please handle these changes manually before proceeding");
    process.exit(1);
  }

  console.log("📦 Committing version artifacts...");
...
📦 Version: ${version}
🔧 Automated: Version embedding artifacts
...
Authored-By: Dave Thompson <dave.thompson@3leaps.net> [@3leapsdave](https://github.com/3leapsdave)`;
  ...
  try {
    ...
    console.log(`✅ Version ${version} artifacts committed successfully`);
  } catch (error) {
    console.error("❌ Failed to commit version artifacts");
    throw error;
  }
}
...
async function versionCommitWorkflow(
  ...
  console.log(`🚀 Version Commit Workflow for v${version}`);
  console.log("");
  ...
  if (!embedOnly) {
    ...
    if (finalStatus.isClean) {
      console.log("");
      console.log("🎉 Version commit workflow completed successfully!");
      console.log(`📦 Version ${version} is ready for release`);
      console.log("✅ Working tree is clean - ready for push operations");
    } else {
      console.log("");
      console.log("⚠️  Warning: Working tree still has uncommitted changes");
...
if (import.meta.main) {
  ...
  versionCommitWorkflow({ embedOnly, commitOnly })
    ...
    .catch((error) => {
      console.error("❌ Version commit workflow failed:", error.message);
      process.exit(1);
    });
}
