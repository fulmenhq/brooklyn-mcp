...
import { loadConfig, type BrooklynConfig } from "../core/config.js";
import { initializeLogging, getLogger } from "../shared/structured-logger.js";
import { BrooklynEngine, type BrooklynEngineOptions } from "../core/brooklyn-engine.js";
import { createMCPStdio, createHTTP } from "../transports/index.js";
  ...
  mcpCmd
    ...
    .action(async (options) => {
      try {
        ...
        if (options.logLevel) cliOverrides.logging = { level: options.logLevel };
...
function setupWebCommands(program: Command): void {
  const webCmd = program
    .command("web")
    .description("Web server commands for monitoring and APIs");
      ...
      try {
        ...
        if (options.port) {
          cliOverrides.transports = {
            http: { port: parseInt(options.port, 10) }
          };
        }
